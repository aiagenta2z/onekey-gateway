...
def main():
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="(deprecated) Image URL (repeatable)",
    )
    ...
    url = ENDPOINT
    try:
        images = list(args.images) + list(args.ref_image_url)
        validate_ref_image_urls(images)
    except ValueError as exc:
        sys.stderr.write(f"{exc}\n")
        sys.exit(2)

    payload = build_payload(args.prompt, images, args.mode)
