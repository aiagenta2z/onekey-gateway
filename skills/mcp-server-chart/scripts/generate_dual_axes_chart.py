...
def validate_required(payload):
    missing = [key for key in ['categories', 'series'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="mcp-server-chart generate_dual_axes_chart")
    ...
    result = router.invoke(
        ...
        api_id="generate_dual_axes_chart",
        data=payload,
    )
