...
def validate_required(payload):
    missing = [key for key in ['title', 'data'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="mcp-server-chart generate_district_map")
