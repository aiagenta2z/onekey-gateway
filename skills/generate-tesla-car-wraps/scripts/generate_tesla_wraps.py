#!/usr/bin/env python3
"""Call Craftsman Agent OneKey Router to generate Tesla Car Wraps."""
...
def build_payload(prompt, images, mode, car_model, output_number):
    return {
        "unique_id": UNIQUE_ID,
        "api_id": API_ID,
        "data": {
...
def main():
    parser = argparse.ArgumentParser(description="Generate Tesla Car Wraps via Craftsman Agent")
    parser.add_argument("--prompt", required=True, help="Text prompt for the wrap")
    parser.add_argument(
        "--images",
        action="append",
        default=[],
    ...
    parser.add_argument("--car-model", default="tesla_model_yl", help="Tesla model (e.g., tesla_model_yl)")
    parser.add_argument("--output-number", type=int, default=1, help="Number of images to generate")
    ...
    payload = build_payload(args.prompt, args.images, args.mode, args.car_model, args.output_number)
