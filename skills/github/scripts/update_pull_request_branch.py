...
def validate_required(payload):
    missing = [key for key in ['owner', 'repo', 'pullNumber'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="github update_pull_request_branch")
    ...
    result = router.invoke(
        ...
        api_id="update_pull_request_branch",
        data=payload,
    )
