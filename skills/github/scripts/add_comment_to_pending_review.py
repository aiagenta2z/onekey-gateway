...
def validate_required(payload):
    missing = [key for key in ['owner', 'repo', 'pullNumber', 'path', 'body', 'subjectType'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="github add_comment_to_pending_review")
    ...
    result = router.invoke(
        ...
        api_id="add_comment_to_pending_review",
        data=payload,
    )
