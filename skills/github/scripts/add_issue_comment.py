...
def validate_required(payload):
    missing = [key for key in ['owner', 'repo', 'issue_number', 'body'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="github add_issue_comment")
