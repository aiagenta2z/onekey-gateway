...
def validate_required(payload):
    missing = [key for key in ['method', 'owner', 'repo', 'pullNumber'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="github pull_request_review_write")
    ...
    result = router.invoke(
        ...
        api_id="pull_request_review_write",
        data=payload,
    )
