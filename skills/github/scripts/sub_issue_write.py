...
def validate_required(payload):
    missing = [key for key in ['method', 'owner', 'repo', 'issue_number', 'sub_issue_id'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="github sub_issue_write")
