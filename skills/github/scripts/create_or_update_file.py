...
def validate_required(payload):
    missing = [key for key in ['owner', 'repo', 'path', 'content', 'message', 'branch'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="github create_or_update_file")
