...
def validate_required(payload):
    missing = [key for key in ['keywords', 'location'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="amap-maps-streamableHTTP maps_around_search")
