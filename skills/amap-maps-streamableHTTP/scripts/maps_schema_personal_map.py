...
def validate_required(payload):
    missing = [key for key in ['orgName', 'lineList'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="amap-maps-streamableHTTP maps_schema_personal_map")
    ...
    result = router.invoke(
        ...
        api_id="maps_schema_personal_map",
        data=payload,
    )
